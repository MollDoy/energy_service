...
![Структура таблицы базы данных](ИИТИАД.drawio.png)  <!-- Здесь можно вставить скрин или схему -->

Таблица содержит следующие поля:
- `id` — уникальный идентификатор записи;
- `plan_GES`    — план гидроэлектростанций
- `plan_AES`    — план атомных электростанций  
- `plan_TES`    — план тепловых электростанций
- `plan_SES`    — план солнечных электростанций
- `plan_VES`   — план ветровых электростанций
- `plan_othe`  — план прочих возобновляемых источников
- `plan_consumption` (объём потребления) — Полный план
- `plan_export` — План эксп.
- `plan_import` — План имп.
- `price_buy` (цена) — Цена покупки
...
### 3.1 `app.py` — назначение и содержание
...
### 3.2  Примеры кода и пояснения

#### 3.2.1 Инициализация приложения и базы данных
```python
app = Flask(__name__)
...
#### 3.2.2 Планировщик автоматического обновления данных
```python
def reload_data_bg():
    cwd = os.getcwd()
...
#### 3.2.2.3 Проверка параметров запросов
```python
...
#### 3.2.2.4 API для ручного обновления данных
```python
...
#### 3.2.2.5 API для получения таблицы данных
```python
@app.route('/api/table')
def api_table():
...
#### 3.2.2.6 Главная страница сайта
```python
@app.route("/")
def index():
    ses = Session()
...
#### 3.2.2.7 Генерация Excel-отчёта
```python
